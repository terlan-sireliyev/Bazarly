  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      <div className="max-w-7xl mx-auto p-4">
        {/* Header */}
        <header className="bg-white rounded-xl shadow-md hover:shadow-lg transition p-6 mb-6">
          <div className="flex flex-col lg:flex-row justify-between gap-4">
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs sm:text-sm text-gray-700">
                ...
                <span className="px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg font-medium">Baxış: {ad.meta.views}</span>
                <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-lg font-medium">Dərc: {ad.meta.publishedAt}</span>
                <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-lg font-medium">Yeniləndi: {ad.meta.publishedAt}</span>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition ${
                    liked ? "bg-rose-500 text-white hover:bg-rose-600" : "border border-gray-300 hover:border-blue-400"
                  }`}
                >
                  {liked ? <FaHeart /> : <FaRegHeart />} {liked ? "Bəyənildi" : "Bəyən"}
                </button>
                <button
                  onClick={() => setSaved((s) => !s)}
                  className={`px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition ${
                    saved ? "bg-blue-600 text-white hover:bg-blue-700" : "border border-gray-300 hover:border-blue-400"
                  }`}
                >
                  {saved ? <FaBookmark /> : <CiBookmark />} {saved ? "Yadda saxlanıldı" : "Yadda saxla"}
                </button>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-md p-4">
              <div className="w-full h-[500px] flex items-center justify-center bg-gray-100 rounded-lg">
                <img src={images[activeImg]} alt={ad.title} className="max-h-[480px] object-contain" />
              </div>
                ...
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`overflow-hidden rounded-md border bg-white transition ${
                      activeImg === idx ? "ring-2 ring-blue-500 ring-offset-2" : "hover:scale-105 hover:border-blue-400"
                    }`}
                  >
